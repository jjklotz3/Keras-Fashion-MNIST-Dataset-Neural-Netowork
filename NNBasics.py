...
plt.imshow(train_images[7], cmap=plt.cm.binary)  #This shows the 7th image in the 'train_images' array
plt.show()                                       #This actually pulls it up on the computer 

#IN ORDER TO RUN THE REST OF THE PROGRAM, EXIT OUT OF THE FIGURE WINDOW!!
...
prediction = model.predict(test_images)       #This will show us all of the output neuron values the model assigns. 
print(prediction[0])                          #Shows neuron values for the first image in test_images
print(class_names[np.argmax(prediction[0])])  #Shows us the largest neuron value for that image and index that value in the class_names array 
